                ...
                Auth.auth().signIn(with: credential) { (result, error) in
                    ...
                    if let user = result?.user {
                        if result?.additionalUserInfo?.isNewUser ?? false {
                            let user = FirestoreUser(id: user.uid, name: user.displayName ?? "", email: user.email ?? "", phoneNumber: user.phoneNumber ?? "", photoURL: "", accountType: "Sitter")
            ...
            if let givenName = fullName.givenName, let familyName = fullName.familyName {
                ...
                self.authenticationService.updateDisplayName(displayName: displayName) { result in
                    ...
                    case .success(let user):
                        print("Successfully updated the user's display name: \(String(describing: user.displayName)).")
